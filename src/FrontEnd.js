...
export default class FrontEnd {
  ...
  createBoard() {
    let board = new Board(
      this.context,
      BoardConfig.MARGIN_LEFT,
      this.statusArea.height + BoardConfig.MARGIN_TOP,
      BoardConfig.WIDTH,
      BoardConfig.HEIGHT
    );
    board.setColumnSelectionHandler((columnIndex) =>
  ...
  createStatusArea() {
    let statusArea = new StatusArea(
      this.context,
      0,
      0,
      this.width,
      StatusAreaConfig.HEIGHT
    );
    statusArea.render(
  ...
  createPlayAgainButton() {
    ...
    let button = new PlayAgainButton(
      this.context,
      buttonX,
      buttonY,
  ...
  processMoveResult(moveResult) {
    ...
    this.statusArea.render(
      indicatorColor,
      this.pickStatusMessage(moveResult.status.value)
    );
    this.board.render(this.game.currentBoard);

    if (
      moveResult.status.value === Constants.MoveStatus.WIN ||
      moveResult.status.value === Constants.MoveStatus.DRAW
    ) {
      this.gameOver = true;
    }
  ...
  pickStatusMessage(status) {
    switch (status) {
      case Constants.GameStatus.WIN:
        return this.game.currentTurn === Constants.PlayerColor.YELLOW
          ? StatusMessages.RED_WIN
          : StatusMessages.YELLOW_WIN;
      case Constants.GameStatus.DRAW:
        return StatusMessages.DRAW;
    }

    return this.game.currentTurn === Constants.PlayerColor.YELLOW
      ? StatusMessages.YELLOW_TURN
      : StatusMessages.RED_TURN;
  }
